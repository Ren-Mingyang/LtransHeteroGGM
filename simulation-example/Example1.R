...
K = 5                      # The number of auxiliary domains
K.A = 3                    # The number of auxiliary domains
M0.A.vec=rep(M0,K)         # The true number of subgroups in each auxiliary domains
...
network.type = "power.law"   # "power.law"
para.list.target = gen.target.para(p, M0, type=network.type, mue=mue0, seed=2025)
para.list.aux = gen.aux.para.overall(n, p, para.list.target, K, K.A, mue=mue0)
...
res.LocalTrans = trans_GGMM(t.data, lambda.t, M, A.data, lambda.A.list, M.A.vec,
                       cn.lam2=seq(1,0.1,length.out=10),
                       cov.method="weight", preselect.aux=5, sel.type="L2",
...
# res.LocalTrans$infor.num.list
res.LocalTrans$t.res[[1]]$delta.vec
res.LocalTrans$t.res[[1]]$lam.preaux
