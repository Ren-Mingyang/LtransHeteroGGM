Init_trans_GGMM = function(t.data, lambda.t, M, A.data, lambda.A.list, M.A.vec,
                           ...
                           int.refit=T, lambda.t.refit=lambda.t, init_GGMM.target=NULL  ){
  
  if(is.null(init_GGMM.target)){
    res.target = GGMPF(lambda.t, t.data, M, initial.selection=initial.selection, trace = trace)
    if(int.refit){
      res.target = GGMPF(lambda.t.refit, t.data, res.target$K_hat, initial.selection=initial.selection, trace = trace)
    }
  } else {
    res.target = init_GGMM.target
  }
